...
def export_weights_targets_template(driver, scorecard_name=None):
    ...
    try:
        ...
        if bulk_ops_button:
            ...
            print("Phase 2: Clicking 'Bulk Operations' button...")
            automation_status["message"] = "Phase 2: Clicking 'Bulk Operations' button..."
            bulk_ops_button.click()
            
            # Wait for page to navigate to bulk_update page (can take 1-2 minutes)
            print("Phase 2: Waiting for navigation to bulk_update page...")
            automation_status["message"] = "Phase 2: Waiting for page to navigate to bulk_update..."
            try:
                WebDriverWait(driver, 120).until(
                    EC.url_contains('bulk_update')
                )
                print(f"Phase 2: Successfully navigated to: {driver.current_url}")
            except TimeoutException:
                print(f"Phase 2: WARNING - URL didn't change to bulk_update. Current URL: {driver.current_url}")
                # Continue anyway in case we're already on the right page
            
            # Additional wait for page elements to fully load
            print("Phase 2: Waiting for page elements to load...")
            time.sleep(5)
            
            # Wait for any loading indicators to disappear
            try:
                WebDriverWait(driver, 30).until_not(
                    EC.presence_of_element_located((By.XPATH, "//*[contains(@class, 'loading') or contains(text(), 'Loading')]"))
                )
            except:
                pass  # No loading indicator found, continue
            
            # Look for "Export Template" option with increased timeout
            automation_status["message"] = "Phase 2: Looking for Export Template option..."
            print("Phase 2: Looking for 'Export Template' link...")
            export_template_link = find_element_by_xpath(driver, "//a[contains(text(), 'Export Template')]", timeout=30)
            if not export_template_link:
                # Try in dropdown menu
                print("Phase 2: First attempt failed, trying alternative selector...")
                export_template_link = find_element_by_xpath(driver, "//*[contains(text(), 'Export Template')]", timeout=30)
            
            if export_template_link:
                driver.execute_script("arguments[0].scrollIntoView(true);", export_template_link)
                time.sleep(1)
                print("Phase 2: Clicking 'Export Template' link...")
                export_template_link.click()
                automation_status["message"] = "Phase 2: Export template job triggered. Waiting for job to start..."
                print("Phase 2: Export template job triggered successfully")
                time.sleep(5)  # Increased wait for job to be created
                return True
            else:
                automation_status["message"] = "Phase 2: Error: Could not find Export Template option"
                print(f"Phase 2: ERROR - Could not find 'Export Template' link on page: {driver.current_url}")
                return False
        else:
...
def wait_for_export_job_and_download(driver, username, export_file_path=None, max_wait_time=300):
    ...
    try:
        ...
        print("Phase 2: Navigating to tracked_jobs page...")
        driver.get("https://settings.ef.uk.com/tracked_jobs")
        
        # Wait for page to navigate (can take time)
        print("Phase 2: Waiting for tracked_jobs page to load...")
        try:
            WebDriverWait(driver, 60).until(
                EC.url_contains('tracked_jobs')
            )
            print(f"Phase 2: Successfully navigated to: {driver.current_url}")
        except TimeoutException:
            print(f"Phase 2: WARNING - URL didn't change to tracked_jobs. Current URL: {driver.current_url}")
        
        # Additional wait for page content to load
        time.sleep(5)
        
        # Wait for page to load - look for table or job elements
        print("Phase 2: Waiting for page elements to appear...")
        try:
            WebDriverWait(driver, 30).until(
                EC.presence_of_element_located((By.XPATH, "//table | //tbody | //*[contains(text(), 'Job')]"))
            )
            print("Phase 2: Page elements loaded successfully")
        except TimeoutException:
            print("Phase 2: WARNING - Could not find table elements, continuing anyway...")
            pass
                ...
                if matching_jobs:
                    ...
                    if status == 'COMPLETE':
                        ...
                        if download_link:
                            ...
                            print("Phase 2: Found download link, clicking...")
                            driver.execute_script("arguments[0].scrollIntoView(true);", download_link)
                            time.sleep(2)
                            download_link.click()
                            print("Phase 2: Download link clicked, waiting for download to complete...")
                            
                            # Wait longer for download to start and complete
                            # Downloads can take time depending on file size and network speed
                            automation_status["message"] = "Phase 2: Waiting for file to download..."
                            time.sleep(10)  # Increased from 5 to 10 seconds
                            
                            # Find downloaded file
                            # Check in specified export path first, then Downloads folder
                            print("Phase 2: Searching for downloaded file...")
                            time.sleep(5)  # Increased from 3 to 5 seconds for additional safety
                            all_files = []
...
def update_looker_config(driver, customer_name, change_reason, scorecard_type):
    ...
    """
    try:
        # Only proceed if scorecard type is Enterprise
        if scorecard_type != 'Enterprise':
            automation_status["message"] = f"Phase 3a: Skipping Looker Config update (scorecard type is '{scorecard_type}', not Enterprise)"
            print(f"Phase 3a: Skipping Looker Config - scorecard type is '{scorecard_type}'")
            return True
        
        automation_status["message"] = "Phase 3a: Navigating to Looker Config..."
        print("Phase 3a: Starting Looker Config update...")
...
def run_automation(form_data):
    ...
    try:
        ...
        # Check if this is a Competitor scorecard - if so, stop here
        ...
        if scorecard_type == 'Competitor':
            ...
            print("=" * 100)
            print("For Competitor scorecards, automation stops after Phase 2.")
            ...
            print("  ⏭️  Phase 3a: Looker Config Update")
            ...
            automation_status["message"] = "Competitor scorecard automation completed! Phase 1 & 2 done. Browser will close in 10 seconds."
            track_step("Finalization", "Automation Complete", "success", "Competitor scorecard - Phases 1 & 2 completed")
            ...
            if driver:
                try:
                    ...
                    pass
            return  # Exit the automation for Competitor scorecards
        
        # Continue with remaining phases for Enterprise scorecards
        # Phase 3a: Update Looker Config (only for Enterprise scorecards)
        ...
        try:
            if scorecard_type == 'Enterprise':
                automation_status["message"] = "Phase 3a: Updating Looker Config..."
                customer_name = form_data.get('customer_name', '')
                ...
                if update_looker_config(driver, customer_name, change_reason, scorecard_type):
                    automation_status["message"] = "Phase 3a: Looker Config updated successfully"
                    print("Phase 3a: Looker Config update completed successfully")
                    phase3a_completed = True
                else:
                    automation_status["message"] = "Phase 3a: Looker Config update failed (but continuing to Phase 4)"
                    print("Phase 3a: Looker Config update failed (but continuing to Phase 4)")
